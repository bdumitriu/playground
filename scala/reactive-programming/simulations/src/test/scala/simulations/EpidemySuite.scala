...
class EpidemySuite extends FunSuite with PrivateMethodTester {

  val decoratedGetRandomRoom = PrivateMethod[EpidemySimulator#Room]('getRandomRoom)
  ...
  test("in 1,000 tries, the random room was always different than current room") {
    val epidemySimulator: EpidemySimulator = new EpidemySimulator
    val person = new epidemySimulator.Person(1)
    for (i <- 1 to 1000) {
      assert(person.getRoom != (person invokePrivate decoratedGetRandomRoom()), "test 1 - i = "+i)
    }
  }
}
