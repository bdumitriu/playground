...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val serverSubscription: Subscription = Future.run() { token: CancellationToken => Future {
      eventLoop(listener, token, handler)
    }}
    Subscription(listenerSubscription, serverSubscription)
  }

  def eventLoop(listener: Listener, token: CancellationToken, handler: Request => Response) {
    while (token.nonCancelled) {
      val nextRequest: Future[(NodeScala.Request, Exchange)] = listener.nextRequest()
      try {
        val request: (NodeScala.Request, Exchange) = Await.result(nextRequest, Duration.Inf)
        Future {
          respond(request._2, token, handler(request._1))
        }
      } catch {
        case e: TimeoutException => // continue
      }
    }
  }
}
