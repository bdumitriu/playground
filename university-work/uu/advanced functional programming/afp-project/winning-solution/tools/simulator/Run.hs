...
data St = St {
        ...
        st_reddeaths :: !Pickups,
        st_blackdeaths :: !Pickups
    }
...
run start_at iters seed max_ant_id m a cs1 cs2
 = case evalState (do_rounds start_at (start_at + iters)) init_state of
    ...
    where init_state = St max_ant_id m a cs1 cs2 (mk_rng seed) 0 0 0 0 0 0
           ...
           = do st <- get
                ...
                let res' = if i `mod` 100 == 0
                             then VProfile {
                                    ...
                                    profileRedDeaths = st_reddeaths st,
                                    profileBlackDeaths = st_blackdeaths st
                                  } : res
                                 else res
                    res'' = if i < start_at
                            then []
                          ...
                          if length (filter (== Just (other_colour c)) xs) >= 5
                           then do st <- get
                                   let Map m = st_map st
                                       ...
                                       reddeaths = if c == Red then st_reddeaths st + 1 else st_reddeaths st
                                       blackdeaths = if c == Black then st_blackdeaths st + 1 else st_blackdeaths st
                                   put $ st { st_map = Map m',
                                              st_ants = Ants a',
                                              st_redscore = redscore,
                                              st_blackscore = blackscore,
                                              st_reddeaths = reddeaths,
                                              st_blackdeaths = blackdeaths
                                            }
...
dump_map (Map m) (Ants a)
    ...
    where map_cell (p, Cell _ _ f rms bms)
                  = VNoAnt p:VFood p f:do_marks p Red rms ++ do_marks p Black bms
