...
--
-- Transforms a proposition from a DNF representation into a Prop representation.
-- 
dnfToProp :: DNF -> Prop
dnfToProp [["true"]]		= Val True
dnfToProp [["false"]]		= Val False
dnfToProp [[x]]			= makeVar x
dnfToProp [xxs]			= Bin Conj (dnfToProp [xs]) (makeVar x)
  where
  x	= last xxs
  xs	= init xxs
dnfToProp xxs			= Bin Disj (dnfToProp xs) (dnfToProp [x])
  where
  x	= last xxs
  xs	= init xxs

--
-- Transforms a string representation of an identifier or a negated identifier to
-- a proposition.
--
makeVar :: String -> Prop
makeVar ('~':xs)	= Un Neg (Var xs)
makeVar x		= Var x