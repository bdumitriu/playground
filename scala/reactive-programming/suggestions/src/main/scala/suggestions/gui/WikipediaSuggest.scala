...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] =
      searchTerms concatRecovered { wikiSuggestResponseStream(_).timedOut(5) }

    // TO IMPLEMENT
    val suggestionSubscription: Subscription = suggestions.observeOn(eventScheduler) subscribe {
      suggestion: Try[List[String]] => suggestion match {
        case Success(value) => suggestionList.listData = value
        case Failure(exception) => status.text = exception.getMessage
      }
    }

    // TO IMPLEMENT
    val selections: Observable[String] =
      button.clicks filter { _ => !suggestionList.selection.items.isEmpty } map { _ => suggestionList.selection.items.head }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections concatRecovered { wikiPageResponseStream(_).timedOut(5) }

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      page: Try[String] => page match {
