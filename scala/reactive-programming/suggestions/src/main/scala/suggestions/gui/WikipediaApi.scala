...
import rx.lang.scala.{Notification, Observer, Observable}
...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      def onNext(observer: Observer[Try[T]])(notification: Notification[T]) = notification match {
        case OnNext(value) => observer.onNext(Success(value))
      ...
      Observable { observer => obs.materialize.subscribe { onNext(observer) _ } }
    }
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] =
      obs.map(requestMethod).map(_.recovered).concat
  }
}
