...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val myServerSubscription = myServer.start("/test") { _.map { case (key, value) => s"$key: ${value.mkString(", ")}\n" }.iterator }
    ...
    val userInterrupted: Future[String] =
      Future.userInput("Type anything and hit ENTER to stop the server...\n") continue { text =>
        s"You entered... ${text.get}"
      }
