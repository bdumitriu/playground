...
class ListsSuite extends FunSuite {
  ...
  test("sum of negative numbers") {
    assert(sum(List(-1, -10, -100)) === -111)
  }

  test("max of negative numbers") {
    assert(max(List(-1, -10, -100)) === -1)
  }

  test("max of identical numbers with higher number second") {
    assert(max(List(10, 100, 100, 10)) === 100)
  }

  test("max of identical numbers with higher number first") {
    assert(max(List(100, 10, 10, 100)) === 100)
  }
}
